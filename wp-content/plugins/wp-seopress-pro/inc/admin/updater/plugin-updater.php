...
//this file is used to update the plugin, provided by the EDD plugin updater
...
class SEOPRESS_Updater {
	...
	/**
	 * Tweak the HTTP request arguments for calls made to the SEOPress store.
	 *
	 * WordPress' default User-Agent embeds the current site URL
	 * ( "WordPress/6.x; https://example.com" ). When a customer's domain contains
	 * a token flagged by a scanner/fuzzer firewall rule (e.g. "fuzz"), the store
	 * edge resets the connection and the PRO package download fails with HTTP 520
	 * / "cURL error 56: Connection reset by peer". The site URL is useless for our
	 * own store requests, so we send a neutral User-Agent without it.
	 *
	...
	public function http_request_args( $args, $url ) {
		...
		// Only touch requests aimed at the SEOPress store.
		if ( empty( $store_host ) || $store_host !== $url_host ) {
			return $args;
		}

		/**
		 * Filters the User-Agent sent to the SEOPress store.
		 *
		 * Defaults to a neutral "WordPress/x.y" without the site URL, to avoid WAF
		 * false positives when the customer domain contains a flagged token (e.g.
		 * "fuzz"). The store already receives the site URL through the EDD request
		 * params and the download token, so it is redundant in the User-Agent.
		 *
		 * @since 10.0.0
		 *
		 * @param string $user_agent The User-Agent sent to the store.
		 * @param string $url        The request URL.
		 */
		$args['user-agent'] = apply_filters( 'seopress_pro_updater_user_agent', 'WordPress/' . get_bloginfo( 'version' ), $url );

		// Preserve the historical SSL verification toggle for the package download,
		// matching both the legacy query-string form and the current /edd-sl/ path form.
		if ( false !== strpos( $url, 'edd_action=package_download' ) || false !== strpos( $url, '/edd-sl/package_download/' ) ) {
	...
	private function api_request( $_action, $_data ) {
		...
		if ( trailingslashit( get_option( 'home' ) ) === $this->api_url ) {
			return false;
		}
	...
	private function get_version_from_remote() {
		$api_params = array(
			...
			'url'         => isset( $this->api_data['url'] ) ? $this->api_data['url'] : get_option( 'home' ),
			'beta'        => $this->beta,
