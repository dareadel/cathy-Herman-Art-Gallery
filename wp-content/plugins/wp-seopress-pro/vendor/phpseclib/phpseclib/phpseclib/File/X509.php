 ...
 */
namespace SEOPressPro\Vendor\phpseclib3\File;
...
use SEOPressPro\Vendor\phpseclib3\Crypt\PublicKeyLoader;
use SEOPressPro\Vendor\phpseclib3\Crypt\Random;
use SEOPressPro\Vendor\phpseclib3\Crypt\RSA;
use SEOPressPro\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use SEOPressPro\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException;
use SEOPressPro\Vendor\phpseclib3\File\ASN1\Element;
    ...
    private function validateSignatureCountable($caonly, $count)
    {
        ...
        switch (\true) {
            case isset($this->currentCert['tbsCertificate']):
                // self-signed cert
                switch (\true) {
                    case !defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $this->currentCert['tbsCertificate']['subject']:
                    case defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->getIssuerDN(self::DN_STRING) === $this->getDN(self::DN_STRING):
                ...
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < count($this->CAs); $i++) {
                        ...
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $ca['tbsCertificate']['subject']:
                            case defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
            ...
            case isset($this->currentCert['tbsCertList']):
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < count($this->CAs); $i++) {
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertList']['issuer'] === $ca['tbsCertificate']['subject']:
                            case defined('SEOPressPro\Vendor\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertList']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
                                $subjectKeyID = $this->getExtension('id-ce-subjectKeyIdentifier', $ca);
                                switch (\true) {
                                    case !is_array($authorityKey):
