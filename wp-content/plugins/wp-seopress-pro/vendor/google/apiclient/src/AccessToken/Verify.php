 ...
 */
namespace SEOPressPro\Vendor\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use SEOPressPro\Vendor\ExpiredException;
use SEOPressPro\Vendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use SEOPressPro\Vendor\Firebase\JWT\JWT;
use SEOPressPro\Vendor\Firebase\JWT\Key;
use SEOPressPro\Vendor\Firebase\JWT\SignatureInvalidException;
use SEOPressPro\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use SEOPressPro\Vendor\Google\Exception as GoogleException;
use SEOPressPro\Vendor\GuzzleHttp\Client;
use SEOPressPro\Vendor\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use LogicException;
use SEOPressPro\Vendor\phpseclib3\Crypt\AES;
...
class Verify
{
    ...
    private function setPhpsecConstants()
    {
        if (filter_var(getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!defined('SEOPressPro\Vendor\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                define('SEOPressPro\Vendor\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!defined('SEOPressPro\Vendor\CRYPT_RSA_MODE')) {
                define('SEOPressPro\Vendor\CRYPT_RSA_MODE', AES::ENGINE_OPENSSL);
            }
        }
    }
}
