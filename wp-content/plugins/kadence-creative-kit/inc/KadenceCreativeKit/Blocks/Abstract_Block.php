...
class Abstract_Block {
	...
	public function __construct() {
		...
		add_filter( 'kadence_creative_kit_blocks_to_generate_post_css', [ $this, 'add_block_to_post_generate_css' ] );
	}
	...
	public function on_init() {
		if ( $this->should_register() ) {
			// Register editor assets before block registration
			$asset_file = KADENCE_CREATIVE_KIT_PATH . 'build/blocks-' . $this->block_name . '.asset.php';
			if ( file_exists( $asset_file ) ) {
				$asset = include $asset_file;
				wp_register_script(
					'kadence-creative-kit-' . $this->block_name . '-editor',
					KADENCE_CREATIVE_KIT_URL . 'build/blocks-' . $this->block_name . '.js',
					$asset['dependencies'],
					$asset['version']
				);
				wp_register_style(
					'kadence-creative-kit-' . $this->block_name . '-editor',
					KADENCE_CREATIVE_KIT_URL . 'build/blocks-' . $this->block_name . '.css',
			...
			if ( file_exists( KADENCE_CREATIVE_KIT_PATH . 'build/blocks/' . $this->block_name . '/style-index.css' ) ) {
				wp_register_style(
					'kadence-creative-kit-' . $this->block_name,
					KADENCE_CREATIVE_KIT_URL . 'build/blocks/' . $this->block_name . '/style-index.css',
					array(),
			...
			register_block_type(
				KADENCE_CREATIVE_KIT_PATH . 'build/blocks/' . $this->block_name . '/block.json',
				[
					'render_callback' => [ $this, 'render_css' ],
					'editor_script'   => 'kadence-creative-kit-' . $this->block_name . '-editor',
					'editor_style'    => 'kadence-creative-kit-' . $this->block_name . '-editor',
					'style'           => 'kadence-creative-kit-' . $this->block_name,
				]
			);
		}
	}
	...
	public function render_scripts( $attributes, $inline = false ) {
		if ( $this->has_style ) {
			if ( ! wp_style_is( 'kadence-creative-kit-' . $this->block_name, 'enqueued' ) ) {
				$this->enqueue_style( 'kadence-creative-kit-' . $this->block_name );
				if ( $inline ) {
					$this->should_render_inline_stylesheet( 'kadence-creative-kit-' . $this->block_name );
				}
			}
		}
		if ( $this->has_script ) {
			if ( ! wp_script_is( 'kadence-creative-kit-' . $this->block_name, 'enqueued' ) ) {
	...
	public function render_css( $attributes, $content, $block_instance ) {
		...
		if ( ! empty( $unique_id ) ) {
			$unique_id       = str_replace( '/', '-', $unique_id );
			$unique_style_id = apply_filters( 'kadence_creative_kit_build_render_unique_id', $unique_id, $this->block_name, $attributes );
			...
			$attributes = apply_filters( 'kadence_creative_kit_' . str_replace( '-', '_', $this->block_name ) . '_render_block_attributes', $attributes, $block_instance );

			$content = $this->build_html( $attributes, $unique_id, $content, $block_instance );
			if ( ! $css_class->has_styles( 'kb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'kadence_creative_kit_render_inline_css', true, $this->block_name, $unique_id ) ) {
			...
			} elseif ( ! wp_is_block_theme() && ! $css_class->has_header_styles( 'kb-' . $this->block_name . $unique_style_id ) && ! is_feed() && apply_filters( 'kadence_creative_kit_render_inline_css', true, $this->block_name, $unique_id ) ) {
	...
	public function do_inline_styles( &$content, $unique_style_id, $css ) {
		if ( apply_filters( 'kadence_creative_kit_render_styles_footer', $this->block_name == 'data' || $this->block_name == 'slide' ) ) {
	...
	public function register_scripts() {
		...
		wp_register_style( 'kadence-creative-kit-' . $this->block_name, KADENCE_CREATIVE_KIT_URL . 'build/blocks/' . $this->block_name . '/style-index.css', [], KADENCE_CREATIVE_KIT_VERSION );
	}
