...
* Fix: Custom SVG library search failing the license check on the patterns server (the request now sends the `site_url` parameter the endpoint expects).
* Tweak: Custom SVG library search now uses an authorized license key, including unified Liquid Web licenses, so it works for sites licensed through Liquid Web.

= 2.8.15 =
Release Date: 27th May 2026
* Fix: Query Loop(Adv) AJAX filter actions not using the correct language month names with PolyLang
* Fix: Query Loop(Adv) layout/design issue on WordPress 7
* Fix: Query Loop(Adv) filter blocks Polylang Pro compatibility bug

= 2.8.14 =
Release Date: 8th April 2026
Fix: Query Loop filter by product attribute.

= 2.8.13.1 =
Release Date: 17th March 2026
* Fix: Fatal error with ACF map fields. 

= 2.8.13 =
Release Date: 12th March 2026
* Fix: Issue with google maps on some posts
* Fix: issue with modal blocks on mobile devices

= 2.8.12 =
Release Date: 11th March 2026
* Fix: Advanced Gallery block not rendering in Dynamic HTML > Post Content
* Fix: Conditional Display Not Working When Set to Archive URL
