<?php
/**
 * Plugin Name: Kadence Theme Kit Pro - Premium addon for the Kadence Theme
...
/**
 * Plugin basename, e.g. "kadence-pro/kadence-pro.php".
 *
 * @since 1.2.0
 */
if (! defined('KTP_PLUGIN_BASENAME')) {
	define('KTP_PLUGIN_BASENAME', plugin_basename(__FILE__));
}
...
add_action( 'plugins_loaded', static function () {
	( new KadenceWP\KadencePro\Harbor\Harbor_Provider() )->register();
}, 3 );
...
final class KTP_Requirements_Check {
	...
	public function __construct() {
		...
		add_action( 'after_setup_theme', [ $this, 'check_and_load' ], 1 );
	}
	...
	private function quit() {
		...
		add_action( 'wp_loaded', [ $this, 'hide_inactive_theme_notice' ] );
	}
	...
	private function load() {
		...
		if ( class_exists( 'Kadence\Theme' ) && ! class_exists( 'Kadence_Theme_Pro' ) ) {
			include_once dirname( $this->file ) . '/class-kadence-theme-pro.php';
		}
		...
		if ( class_exists( 'Kadence_Theme_Pro' ) ) {
			...
			add_action( 'after_setup_theme', [ $this, 'bootstrap' ], 4 );
	...
	public function admin_head() {
		add_action( 'admin_notices', [ $this, 'admin_notice_need_kadence_theme' ] );
	}
	/**
	 * Hide Notice
	 */
